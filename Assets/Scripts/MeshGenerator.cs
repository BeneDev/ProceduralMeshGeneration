...
public static class MeshGenerator {
    ...
    public static Mesh GenerateTerrainMesh(float[,] heightMap, float scale, AnimationCurve curve, float verticalScale = 1f)
    {
        ...
        for (int y = 0; y <= height; y++)
        {
            for (int x = 0; x <= width; x++)
            {
                ...
                {
                    vertices[i].y = curve.Evaluate(heightMap[x, y]) * verticalScale;
                } 
                i++;
            }
        }
